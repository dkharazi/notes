...
- An interpreter is a program that executes a given language to receive some desired output
- A compiler is a program that translates a source language into a destination language
- Bytecode is platform dependent, whereas machine code is platform dependent
- Statically-typed languages verify data types at compile-time
- Dynamically-typed languages verify data types at run-time
- Statically-typed languages are generally faster than dynamically-typed languages
- This is because statically-typed languages give the compiler much more information
- Compilers of dynamically-typed languages exist
- However, these compilers generally include so many more additional checks that lead to a slower destination language (e.g. bytecode)
